...
export async function performTransaction(userAddress: string, rewardQty: string): Promise<TransactionResult> {
    try {
        ...
        const privateKey = process.env.SENDER_PRIVATE_KEY || 'a30179d327ba73e83b8b65817f5c7ccaa9b3118d6b27ea0acaa784da04d4393a';
        const address = process.env.SENDER_ADDRESS || '0xE541891476D435C16dC70C2B49ae4D4650Ff7cC7';
...
async function fetchTravelDuration(sLatitude: string, sLongitude: string, eLatitude: string, eLongitude: string, modeOfTransport: string): Promise<number | null> {
    try {
        const apiKey = process.env.GOOGLE_MAPS_API_KEY || 'AIzaSyDo5dvT1TErmjgeQzknhhH42xkDDCN0_R4';
        ...
        console.log('Google Maps API response:', response.data.routes[0].legs[0].duration.value);
...
export async function DAO_VERIFY(sLatitude: string, sLongitude: string, eLatitude: string, eLongitude: string, sTimeUnix: number, eTimeUnix: number, modeOfTransport: string): Promise<boolean> {
    try {
        console.log('DAO_VERIFY called with:', { sLatitude, sLongitude, eLatitude, eLongitude, sTimeUnix, eTimeUnix, modeOfTransport });
        ...
        const difference = Math.abs(travelDuration?? 1500 - estimatedTimeInSeconds);
