...
export const getRidingFromWalletAddress = query({
  ...
  handler: async (ctx, args) => {
    const transactions = await ctx.db.query("transactions").filter((q)=> q.eq(q.field("walletAddress"), args.walletAddress)).collect()
...
export const updateTransaction = mutation({
  ...
  handler: async (ctx, args) => {
    const transaction = await ctx.db.query("transactions").filter((q)=> q.eq(q.field("walletAddress"), args.walletAddress)).collect();
